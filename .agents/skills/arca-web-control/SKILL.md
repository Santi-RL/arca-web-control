...
5. No ejecutar `emit-prepared-invoice`: el manifiesto vigente mantiene la emisión deshabilitada hasta una nueva validación visible y promoción humana.
6. Al revalidar la implementación irreversible en una versión futura, exigir exactamente `EMITIR`, no releer ni sustituir el job y bloquear reintentos ante estado `unknown`.
