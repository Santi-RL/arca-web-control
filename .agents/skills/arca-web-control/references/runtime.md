...
Perfiles, jobs reales, sesiones, aprendizaje, logs, ledger, capturas y PDFs viven en `%LOCALAPPDATA%\ManejoARCA`. Cada job real debe ser un archivo `.json` regular bajo `jobs\private`; la sesión y el MCP rechazan cualquier otra ubicación o enlace. La raíz no es configurable mediante `ARCA_RUNTIME_ROOT`, y `NODE_ENV=test` no se admite en comandos operativos. En un job, `outputDir` es `"."` o una subcarpeta relativa de `downloads`; nunca una ruta absoluta. Los artefactos crudos nunca se versionan. Una contribución solo puede incluir código, documentación y fixtures totalmente ficticios, creados desde cero y no derivados de datos reales.
