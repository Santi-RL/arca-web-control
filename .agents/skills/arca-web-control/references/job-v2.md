...
`invoice-services-single-item` cubre un receptor identificado por CUIT. `invoice-services-single-item-consumidor-final-anonimo` separa el Consumidor Final sin identificar. Ambas capacidades se limitan a Factura C, concepto Servicios, moneda local, un ítem y Chrome visible. Tienen madurez `controlled_irreversible`: permiten emitir sobre una preparación verificada después de la confirmación humana exacta `EMITIR`, pero no habilitan modo oculto.
...
- El intake conversacional admite enteros como `1500000` o `1.500.000` y los normaliza a dos decimales. El job persistido mantiene `amount` como decimal exacto con dos dígitos. Si llega como número JSON, debe tener como máximo dos decimales y se rechaza en lugar de redondearse.
...
- Al validar un receptor identificado contra ARCA, exigir un único CUIT visible y que sus once dígitos coincidan exactamente con `recipientCuit`; no extraer dígitos de texto arbitrario. Siempre exigir un nombre legal visible no vacío y mostrarlo junto con el CUIT en el resumen. Si `recipientName` está presente, compararlo conservando igual cantidad y orden de términos, normalizando mayúsculas, tildes y puntuación (`S.A.` equivale a `SA`). Si aparece una forma societaria, debe estar presente en ambos nombres y coincidir exactamente. Solo se admite una inserción, eliminación, sustitución o transposición de un carácter en un único término no numérico de al menos seis caracteres; bloquear omisiones, agregados, reordenamientos, formas societarias múltiples y cualquier otra diferencia. Ante una discrepancia, devolver el nombre canónico y el CUIT con `ARCA_RECIPIENT_NAME_CONFIRMATION_REQUIRED` para que el agente solicite una confirmación concreta.
...
- Llegar a un resumen verificado no autoriza por sí solo la emisión. Después de mostrar todos los datos —incluido el nombre legal y CUIT del receptor identificado—, un mensaje humano nuevo exactamente igual a `EMITIR` permite ejecutar una sola vez `emit-prepared-invoice` sobre el mismo `preparedInvoiceId` y la misma sesión visible. El carril de revalidación queda reservado a desarrollo para una versión todavía pendiente y nunca forma parte del camino operativo normal.
